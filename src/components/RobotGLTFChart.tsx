...
function NeonArm({ target, base = new THREE.Vector3(-3.0, -1.05, 0.08) }: {
  ...
  useFrame(() => {
    ...
    shoulder.current.rotation.y = THREE.MathUtils.lerp(
      shoulder.current.rotation.y, Math.sin(performance.now()/1200)*0.1, 0.06
    );
  ...
  return (
    <group position={base.toArray()}>
      <mesh position={[0,-0.28,0]}><cylinderGeometry args={[0.14,0.18,0.55,24]} />
...
/** ===== тело-робот (GLTF, CC0) — чисто визуал, Idle-анимация ===== */
function BotBody() {
  // CDN с CC0-моделью RobotExpressive (Khronos / modelviewer)
...
/** ===== частицы-вспышка по сделке ===== */
function Particles({ at }: { at: THREE.Vector3 | null }) {
  const ref = useRef<THREE.Points>(null);
  const [tBorn, setTBorn] = useState(0);
  useEffect(()=>{ if(at) setTBorn(performance.now()); },[at]);

  const { pos } = useMemo(() => {
    const N=72; const arr=new Float32Array(N*3);
    for (let i=0;i<N;i++){
...
/** ===== основная сцена внутри правого «кармана» ===== */
function Scene() {
  ...
  const shiftX = 2.4 - last.x; // ставим последнюю точку ближе к правому центру
  ...
  const burst = useMemo(()=> trade ? new THREE.Vector3(trade.x+shiftX, trade.y, 0) : null, [trade,shiftX]);
  ...
  return (
    <>
      ...
      {/* Единый карман для графика + робота */}
      <group position={[2.2,0.9,0]} rotation={[-Math.PI*0.12, Math.PI*0.07, 0]}>
        ...
        {/* поток цены (подвинут внутрь кармана) */}
        ...
        {/* 3D-робот (тело, Idle-анимация) */}
        <Suspense fallback={null}>
          <BotBody/>
        </Suspense>

        {/* неоновая рука, реально «кликающая» по свечам */}
